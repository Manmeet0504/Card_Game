...
class Player:
    ...
    def add_cards(self,new_cards):
        if type(new_cards) == type([]):
            ...
            #When we have single card to add
...
for x in range(26):
    player_one.add_cards(new_d.deal_one())
    player_two.add_cards(new_d.deal_one())
    ...
    while at_war:
        if player_one_cards[-1].value > player_two_cards[-1].value :
            ...
            if len(player_one.all_cards) < 5:
                print("Print 1 can't declare war")
                print("Player 2 WINS !!")
                game_on = False
                break
                
            elif len(player_two.all_cards) < 5:
                print("Print 2 can't declare war")
                print("Player 1 WINS !!")
                game_on = False
                break
